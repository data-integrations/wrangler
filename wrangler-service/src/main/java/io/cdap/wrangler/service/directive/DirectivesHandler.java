    ...
    respond(request, responder, () -> {
      ...
      if (!jexlAllowlistEnabled && userAllowlist != null) {
        throw new BadRequestException(
          "Updates to JEXL allowlist in the wrangler directives config is disabled by administrator.");
      }
      ...
      if (!jexlAllowlistEnabled) {
        ...
        JsonObject existingJson = existingConfig.toJson().getAsJsonObject();
        JsonObject newJson = config.toJson().getAsJsonObject();
        existingJson.add(DirectiveConfig.EXCLUSIONS_KEY, newJson.get(DirectiveConfig.EXCLUSIONS_KEY));
        existingJson.add(DirectiveConfig.ALIASES_KEY, newJson.get(DirectiveConfig.ALIASES_KEY));
        config = GSON.fromJson(existingJson, DirectiveConfig.class);
      }
