 ...
 */
 ...
 public final class TestingRig {
   ...
   public static RecipePipeline<List<Row>, List<Row>, Row> pipeline(Class<? extends Directive> directive, TestRecipe recipe)
   ...
   public static List<Row> execute(String[] recipe, List<Row> rows) throws Exception {
     ...
     for (String directive : recipe) {
       testRecipe.add(directive);
     }
 
     RecipePipeline<List<Row>, List<Row>, Row> pipeline =
       pipeline(io.cdap.directives.aggregates.AggregateStats.class, testRecipe);
 
     List<List<Row>> input = new ArrayList<>();
     input.add(rows);
 
     return pipeline.execute(input).get(0); // assume one batch of rows
   }
