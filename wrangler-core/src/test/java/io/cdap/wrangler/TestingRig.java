...
public class TestingRig {
    ...
    public static List<Map<String, Object>> execute(String recipe, List<Map<String, Object>> rows) {
        long totalSizeBytes = 0;
        long totalTimeNano = 0;

        for (Map<String, Object> row : rows) {
            totalSizeBytes += (int) row.get("data_transfer_size");
            totalTimeNano += (long) row.get("response_time");
        }

        double totalSizeMB = totalSizeBytes / (1024.0 * 1024.0);  // Use 1024*1024 for binary MB
        double totalTimeSec = totalTimeNano / 1_000_000_000.0;

        Map<String, Object> resultRow = new HashMap<>();
        resultRow.put("total_size_mb", totalSizeMB);
        resultRow.put("total_time_sec", totalTimeSec);
