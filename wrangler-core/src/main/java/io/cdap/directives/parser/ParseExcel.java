...
public class ParseExcel implements Directive, Lineage {
  ...
  public List<Row> execute(List<Row> records, final ExecutorContext context) {
    ...
    try {
        ...
        if (idx != -1) {
          ...
          if (object instanceof byte[]) {
            ...
            String errorMessage = String.format(
                "Column '%s' should be of type 'byte array' or 'ByteBuffer'.", column);
          ...
          if (bytes != null) {
            ...
            if (excelsheet == null) {
              String errorMessage = String.format(
                  "Failed to extract sheet '%s' from the excel. " + "Sheet '%s' does not exist.",
                  sheet, sheet);
              throw ErrorUtils.getProgramFailureException(
                  new ErrorCategory(ErrorCategory.ErrorCategoryEnum.PLUGIN), errorMessage,
                  errorMessage, ErrorType.USER, false, null);
      ...
      String errorMessage = String.format("Error parsing Excel file with directive '%s', %s: %s",
