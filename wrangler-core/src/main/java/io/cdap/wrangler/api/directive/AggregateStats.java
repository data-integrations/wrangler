...
public class AggregateStats implements Directive {
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        sourceSizeColumn = (ColumnName) args.value("source-size-column");
        if (sourceSizeColumn == null) {
            throw new DirectiveParseException(
                    "aggregate-stats requires a source size column");
        }

        sourceTimeColumn = (ColumnName) args.value("source-time-column");
        if (sourceTimeColumn == null) {
            throw new DirectiveParseException(
                    "aggregate-stats requires a source time column");
        }

        targetSizeColumn = (ColumnName) args.value("target-size-column");
        if (targetSizeColumn == null) {
            throw new DirectiveParseException(
                    "aggregate-stats requires a target size column");
        }

        targetTimeColumn = (ColumnName) args.value("target-time-column");
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context) {
        ...
        ByteSize totalByteSize = new ByteSize(totalSize, "MB");
        result.add(targetSizeColumn.value(), totalByteSize.getOriginalValue());
