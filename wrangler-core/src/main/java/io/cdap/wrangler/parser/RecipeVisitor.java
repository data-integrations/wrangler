...
public final class RecipeVisitor extends DirectivesBaseVisitor<RecipeSymbol.Builder> {
  ...
  /**
   * This visitor method extracts the value field, which can be a string, number,
   * column,
   * boolean, byte size, or time duration. It creates the appropriate token type
   * and adds
   * it to the <code>TokenGroup</code>.
   */
  @Override
