    ...
    */
    ...
    public RecipeSymbol.Builder visitPropertyList(DirectivesParser.PropertyListContext ctx) {
      ...
      for (DirectivesParser.PropertyContext property : properties) {
        ...
        Token token;
        if (property.value().numberValue() != null) {
          token = new Numeric(new LazyNumber(property.value().numberValue().getText()));
        } else if (property.value().boolValue() != null) {
          token = new Bool(Boolean.parseBoolean(property.value().boolValue().getText()));
        } else if (property.value().stringValue() != null) {
          String text = property.value().stringValue().getText();
          token = new Text(text.substring(1, text.length() - 1));
        } else if (property.value().columnValue() != null) {
          token = new Text(property.value().columnValue().getText());
        } else if (property.value().byteSizeValue() != null) {
          token = new Text(property.value().byteSizeValue().getText());
        } else if (property.value().timeDurationValue() != null) {
          token = new Text(property.value().timeDurationValue().getText());
        } else {
          token = new Text("");
        }
      ...
      return builder;
    } // ✅ <-- This is the correct and only closing brace for this method
