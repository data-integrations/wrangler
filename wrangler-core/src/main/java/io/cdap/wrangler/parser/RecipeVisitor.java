...
public final class RecipeVisitor extends DirectivesBaseVisitor<RecipeSymbol.Builder> {
  ...
  /**
   * A Directive can include byte size arguments. This visitor method
   * extracts the byte size and creates a token of type <code>ByteSize</code>.
   */
  @Override
  public RecipeSymbol.Builder visitByteSizeArg(DirectivesParser.ByteSizeArgContext ctx) {
    ...
    // If you have direct access to the group:
    // group.add(byteSize);
    // Otherwise, try casting:
    builder.addToken((Token) byteSize);
    return builder;
  }

  /**
   * A Directive can include time duration arguments. This visitor method
   * extracts the time duration and creates a token of type
  ...
  public RecipeSymbol.Builder visitTimeDurationArg(DirectivesParser.TimeDurationArgContext ctx) {
    ...
    // Direct casting to Token
    builder.addToken((Token) timeDuration);
    return builder;
  }

  /**
   * This method handles different types of values including integers, decimals,
   * byte sizes, and time durations.
   */
  @Override
  public RecipeSymbol.Builder visitValue(DirectivesParser.ValueContext ctx) {
    if (ctx.INTEGER() != null) {
      // Existing integer handling
      // Example: builder.addToken(new Numeric(new
      // LazyNumber(ctx.INTEGER().getText())));
    } else if (ctx.DECIMAL() != null) {
      // Existing decimal handling
      // Example: builder.addToken(new Numeric(new
      // LazyNumber(ctx.DECIMAL().getText())));
