...
/**
 * Simplified tests for {@link TimeDuration} class.
 */
public class TimeDurationTest {
  private static final double DELTA = 0.001;

  /**
   * Tests basic time duration parsing and conversion.
   */
  @Test
  public void testBasicTimeValues() {
    // Test seconds with decimal point
    TimeDuration secondDuration = new TimeDuration("1.5s");
    Assert.assertEquals(1500.0, secondDuration.getMillis(), DELTA);
    Assert.assertEquals(1.5, secondDuration.getSeconds(), DELTA);
    Assert.assertEquals("s", secondDuration.getUnit());
    Assert.assertEquals(1.5, secondDuration.getNumericValue(), DELTA);
  }

  /**
   * Tests the convertToUnit method.
   */
  @Test
  public void testConvertToUnit() {
    ...
    Assert.assertEquals(10000.0, toMillis.getNumericValue(), DELTA);
    
    TimeDuration toMinutes = tenSecondsDuration.convertToUnit("m");
    Assert.assertEquals("m", toMinutes.getUnit());
    Assert.assertEquals(10.0 / 60, toMinutes.getNumericValue(), DELTA);
  }
}
