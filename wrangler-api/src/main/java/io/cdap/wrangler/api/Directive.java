...
/**
 * Directive is a user defined directive. DIE - Define, Initialize & Execute -
 * Pattern
 *
...
public interface Directive extends Executor<List<Row>, List<Row>>, EntityMetrics {
   ...
   *
   * <p>
   * This constant should be used when defining a plugin. An example
   * of such usage is as follow:
   * </p>
   *
   * <code>
   *   @Plugin(type = Directive.Type)
   ...
   * definitions. Each token definition consists of {@code name},
   * {@code TokenType}
   ...
   *
   * <p>
   * The {@code UsageDefinition} provides different methods to {@code define},
   * and as well generate {@code usage} based on the definition.
   * </p>
   *
   * <p>
   * This method is invoked by the framework at the time of creating an executable
   * directive that will be added to the {@link RecipePipeline}. It's generally
   * during
   ...
   * </p>
   *
   * {@code TokenType} supports many different token types that can be used within
   * the
   * usage definition.
   *
   * @return A object of {@code UsageDefinition} containing definitions of each
   *         argument
  ...
  /**
   * This method provides a way to emit metrics from the Directive. Metadata about
   * each metric to be emitted can be
   * returned and used in the metrics emission logic elsewhere.
   * 
   * @return List of metrics ({@link EntityCountMetric}s) emitted by this
   *         directive
   */
  @Override
